...
//Kata 5: Number of trailing zeros of N!
...
describe(
    'Test',()=>
        test("kata: Number of trailing zeros of N!",()=>{
...
function generateHashtag (string) {
   ...
   describe(
    'Test',()=>
        test("kata: The Hashtag Generator",()=>{
...
//Kata 5: Where my anagrams at?

function anagrams(word, words) {
    const checker = (str1,str2)=>{
            let check = (str1.split('').sort().join('')==str2.split('').sort().join(''));
            return check
          }
          let res = [];
          for(const item of words){
            (checker(word,item))?res.push(item):null;
          }
          return res;
  }
  ...
  describe(
    'Test',()=>
        test("kata: Where my anagrams at?",()=>{
            expect(anagrams('ab')).toBe(['ab', 'ba'])
            expect(anagrams('abba')).toBe(['aabb', 'bbaa', 'abab', 'baba', 'baab'])
        })
)
